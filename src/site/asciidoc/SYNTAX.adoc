...
A part entry is a sequence of strokes with associated default parameters; its relationships with strokes and notes are described in the
figure above.
...
A tempo change meta message is sent with a given value to the channel associated with this part entry.
...
If a part entry is played with this groove, the first sixteen notes will be length of 28 MIDI ticks, while usually sixteenth notes have only 24 MIDI ticks (currently, a whole note is fixed to 384 MIDI ticks in symfonion). And 30 is added to the original velocity of the note.
