...
NOTE: The top-level `$patterns` section is a **legacy (compat-only) feature** supported for backward compatibility. New files should define patterns inline inside each bar's `$parts` array in `$sequence` (see <<_sequence>>).

In the legacy approach, the `$patterns` section is a dictionary at the top level of the symfonion file that defines named patterns which are referred to from inside the `$sequence` section.
...
As discussed so far, users can write a bar's part entry like below.
